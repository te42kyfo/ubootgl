...
void main(void) {
  ...
  float fnowFrame = 0;
  float blendFactor = modf(frame, fnowFrame);
  int nowFrame = int(fnowFrame);
  int nextFrame = nowFrame + 1;

  nowFrame = nowFrame % frameCount;
  nextFrame =nextFrame % frameCount;

  ivec2 nowFrameGridIdx =
      ivec2(mod(nowFrame, frameGridSize.x), frameGridSize.y - 1 -  nowFrame / frameGridSize.x);
  ivec2 nextFrameGridIdx =
      ivec2(mod(nextFrame, frameGridSize.x), frameGridSize.y - 1 -  nextFrame / frameGridSize.x);
...
vec2 nowTexCoord = out_UV  / frameGridSize +
  ...
  if (out_PlayerColor < 0) {
    ...
    vec4 texColor =
        mix(textureLod(tex, nowTexCoord, 1), textureLod(tex, nextTexCoord, 1), blendFactor);
