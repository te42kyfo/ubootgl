...
void draw(entt::registry &registry, glm::mat4 PVM, float magnification) {
  ...
  auto spriteView = registry.view<CoItem, CoSprite>();

  if (spriteView.empty())
    return;

  map<Texture *, vector<entt::entity>> texs;
  for (auto entity : spriteView) {
    texs[spriteView.get<CoSprite>(entity).tex].push_back(entity);
  }
  ...
  for (auto tex : texs) {
    ...
    for (auto entity : texs[tex.first]) {
      auto item = spriteView.get<CoItem>(entity);
      auto sprite = spriteView.get<CoSprite>(entity);
      ...
      if (screenPos.x > -1.1 && screenPos.x < 1.1 && screenPos.y > -1.1 &&
        ...
        frame_buf.push_back(sprite.frame);
        frame_buf.push_back(sprite.frame);
        frame_buf.push_back(sprite.frame);
        frame_buf.push_back(sprite.frame);
        frame_buf.push_back(sprite.frame);
    ...
    GL_CALL(glBindTexture(GL_TEXTURE_2D, tex.first->tex_id));
    ...
    GL_CALL(glUniform2i(item_shader_frameGridSize_uloc, tex.first->nx, tex.first->ny));
